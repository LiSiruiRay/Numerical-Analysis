...
def Newton_test_speed(f, fp, a, tol=1e-10):
...
"""
    ...
    while True:
        ...
        if fabs(a) < tol: # distance between a and 0
            break
        else:
            a = newa
        counter += 1
...
def bisection_test_speed(f, a, b, tol=1e-10):
...
"""
    ...
    counter = 0
    while abs((a + b) / 2.) > tol: # distance between 0 and answer
...
if __name__ == "__main__":
    bisection_test_speed(x_e_n_x, -1., 0.5, tol=1e-10)
    regula_falsi_test_speed(x_e_n_x, a=-1., b=0.5, tol=1e-10)
    Newton_test_speed(x_e_n_x, derivative_xenx, a=-1.)
